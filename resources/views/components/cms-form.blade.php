...
<form action="{{ $action }}" method="POST" {{ $hasFile ? 'enctype=multipart/form-data' : '' }} class="enhanced-form">
    @csrf
                ...
                <div class="file-upload-wrapper"
                    ...
                    @if($showFilePreview)
                        <div class="file-preview-zone mt-3">
                            <div id="{{ $uniqueId }}-preview" class="preview-container d-flex flex-wrap gap-2"></div>
...
.preview-container, .existing-files {
    gap: 1rem;
}
.file-wrapper {
    position: relative;
