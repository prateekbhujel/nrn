...
@section('title', db_trans('languages.add_language'))
...
@section('content')
    ...
    <div class="card">
        <div class="card-header">
            <h4>{{ db_trans('languages.add_language') }}</h4>
        </div>
        <div class="card-body">
            @php
                $fields = [
            ...
            <x-cms-form action="{{ route('admin.languages.store') }}" :fields="$fields"
                submitText="{{ db_trans('form.add_language') }}" buttonPosition="right" />
        </div>
    </div>
@endsection
